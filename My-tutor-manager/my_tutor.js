module.exports = function Mytutor(pool) {
  ...
  async function getName(name) {
    ...
    let get_all_names = await pool.query("SELECT * FROM students WHERE student_name = $1", [student]);
    if (get_all_names.rows.lenght !== 0) {
        error_message = ''
      return true;
    } else {
      await pool.query("INSERT INTO students (student_name) VALUES ($1)", [student]);
    }
  }
